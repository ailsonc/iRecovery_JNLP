...
/**
 * File: jWMI.java Date: 12/21/09 Author: Shaun Henry Copyright Henry Ranch LLC 2009-2010. All rights reserved. http://www.henryranch.net A java bridge for
 * querying the WMI interface.
 * 
...
public class JWMI {
	 ...
	 *            a comma separated list of the WMI fields to be collected from the query results. <br>
	 *            i.e. "Model"
	...
	private static String getVBScript(final String wmiQueryStr, final String wmiCommaSeparatedFieldName) {
		String vbs = "Dim oWMI : Set oWMI = GetObject(\"winmgmts:\")" + JWMI.CRLF;
		vbs += "Dim classComponent : Set classComponent = oWMI.ExecQuery(\"" + wmiQueryStr + "\")" + JWMI.CRLF;
		vbs += "Dim obj, strData" + JWMI.CRLF;
		vbs += "For Each obj in classComponent" + JWMI.CRLF;
		final String[] wmiFieldNameArray = wmiCommaSeparatedFieldName.split(",");

		for (final String element : wmiFieldNameArray) {
			vbs += "  strData = strData & obj." + element + " & VBCrLf" + JWMI.CRLF;
		}
		vbs += "Next" + JWMI.CRLF;
		vbs += "wscript.echo strData" + JWMI.CRLF;
		return vbs;
	}
	 ...
	 */
	private static String getEnvVar(final String envVarName) throws Exception {
		final String varName = "%" + envVarName + "%";
		final String envVarValue = JWMI.execute(new String[] { "cmd.exe", "/C", "echo " + varName });
	 ...
	 */
	private static void writeStrToFile(final String filename, final String data) throws Exception {
		try {
			...
			Thread.sleep(1000);
			JWMI.writeStrToFile(filename, data);
		}
	}
	 ...
	 */
	public static String getWMIValue(final String wmiQueryStr, final String wmiCommaSeparatedFieldName) throws Exception {
		final String vbScript = JWMI.getVBScript(wmiQueryStr, wmiCommaSeparatedFieldName);
		final String tmpDirName = JWMI.getEnvVar("TEMP").trim();
		final String uriFileName = tmpDirName + File.separator + "jwmi.vbs";

		JWMI.writeStrToFile(uriFileName, vbScript);

		final String output = JWMI.execute(new String[] { "cmd.exe", "/C", "cscript.exe", uriFileName });

		Files.deleteIfExists(Paths.get(uriFileName));
	 ...
	 * @throws an
	 *             Exception upon encountering a problem
	 */
	private static String execute(final String[] cmdArray) throws Exception {
	...
	public static void executeDemoQueries() {
		try {
			...
			// System.out.println(getWMIValue("Select * from Win32_Service WHERE State = 'Stopped'",
			// "Name"));
			...
			// this is a failing case where the Win32_Service class does not contain
			// the 'Name' field
			// System.out.println(getWMIValue("Select * from Win32_Service",
			// "Name"));
		} catch (final Exception e) {
	...
	public static void main(final String[] argv) {
		try {
			if (argv.length == 0) {
				JWMI.executeDemoQueries();
			} else if (argv.length == 2) {
				System.out.println(JWMI.getWMIValue(argv[0], argv[1]));
			} else {
	...
	// public static final String CLASS_Win32_Bus Win32_CacheMemory =
